...
export function resolveComponent(
  ...
  // Container components take a FieldProps superset; FieldRenderer routes them
  // to ContainerField, which re-types the props before rendering.
  const asFieldComponent = (component: RegisterableComponent) =>
    component as React.ComponentType<FieldProps>
  ...
  if (
    ...
    return asFieldComponent(registry[field.meta.component]!)
  }
  // 3. field.type key in the merged registry
  if (registry[field.type]) {
    return asFieldComponent(registry[field.type]!)
  }
  // 4. field.type key in the default registry
  if (defaultRegistry[field.type]) {
    return asFieldComponent(defaultRegistry[field.type]!)
  }
